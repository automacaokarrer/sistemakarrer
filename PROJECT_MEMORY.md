...
- Última versão Cloudflare validada nesta data: `a1224044-c4e2-4c56-bbb6-4abc5f14327f`.
- Commit de código correspondente: `4f22339`.
...
- Durante conversas com responsável humano e status diferente de `resolved`, a Luna opera em modo passivo: observa mensagens recebidas e envios humanos bem-sucedidos, usa no máximo as 12 mensagens recentes para atualizar fatos, pendências e resumo no D1 e não chama a Z-API nem cria mensagens de saída. A gravação do resumo compara `through_message_at` para impedir que uma execução antiga sobrescreva memória mais nova.
...
- A migration `0008_luna_ai.sql` cria fatos, análises de documentos, pendências, resumos compactos e métricas de execução. Ela foi validada apenas no D1 local e ainda não foi aplicada em produção.
...
- A migration `0008_luna_ai.sql` existe e passou no D1 local, mas permanece pendente no D1 remoto até a publicação autorizada da integração Luna.
...
- A integração Luna está implementada localmente e passou por typecheck, build, migration local, 40 testes unitários, 20 cenários E2E em desktop/celular, auditoria de dependências de produção sem vulnerabilidades e uma chamada mínima bem-sucedida à agente real, sem dados de clientes. Os dois valores OpenAI foram preenchidos apenas no `.env` local ignorado pelo Git e serão transportados como secrets do Worker sem exposição. Antes de publicar, aplicar `0008_luna_ai.sql` no D1 remoto.
