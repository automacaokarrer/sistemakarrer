...
- A migration `0014` passou no D1 local. A validação local atual passou por 94 testes unitários, typecheck e build; os 38 cenários E2E chegaram ao fim sem falhas impressas em desktop/celular, mas o runner precisou ser interrompido depois porque o servidor de desenvolvimento permaneceu aberto. O responsável autorizou incluir na mesma publicação as mudanças locais anteriores de envio múltiplo de anexos. Ainda não houve aplicação remota da migration, commit, push ou deploy destas alterações. Próximo passo: aplicar a migration remota antes do Worker e validar produção sem enviar mensagem real de teste.
