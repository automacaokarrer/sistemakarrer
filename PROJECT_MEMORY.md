...
- Última versão Cloudflare validada nesta data: `59c53753-1c92-4949-a533-0f4a2771b272`.
- Commit de código correspondente: `c2ea72b`.
...
- Envio pelo chat de imagens JPG/PNG/WebP, documentos PDF/Office/CSV/TXT de até 10 MB e áudios gravados no navegador, com armazenamento privado no R2 e envio em Base64 pela Z-API. O compositor aceita até dez anexos por seleção, mostra prévias e envia os arquivos em sequência na mesma conversa; a legenda acompanha o primeiro anexo compatível.
- Mensagens de texto enviadas pelo CRM podem ser editadas via Z-API em até 15 minutos; mensagens enviadas pelo CRM podem ser apagadas para todos em até dois dias. A interface confirma a exclusão, atualiza o histórico em tempo real e informa quando a Z-API não pode assegurar a remoção no aparelho do destinatário.
...
- Quando um envio é recusado por diferença no formato brasileiro do número, o Worker consulta o número canônico confirmado pelo WhatsApp e repete uma única vez somente se ele representa o mesmo destinatário (variação do nono dígito). Endereços privados `@lid` são preservados, nunca convertidos em suposto número telefônico.
...
- Em 17 de setembro de 2026, foram implementados localmente edição de texto enviado pelo CRM e pedido de exclusão de mensagens enviadas para todos, pela Z-API, com rotas autenticadas `PATCH`/`DELETE /api/conversations/:conversationId/messages/:messageId`, permissão de Chat, auditoria sem conteúdo, serialização de alterações, atualização em tempo real e tombstone no histórico. Edição usa `send-text` com `editMessageId` original dentro de 15 minutos; exclusão usa `DELETE /messages` com `owner=true` dentro de dois dias. A aceitação da Z-API não garante que a remoção tenha se concluído no aparelho do destinatário. A migration local `0014_message_actions.sql` registra edição, exclusão, o endereço efetivamente usado no envio e divergência de destinatário. Mensagens históricas sem destinatário original comprovado não podem ser editadas nem apagadas pelo CRM; endereços privados `@lid` também ficam fora dessas duas ações até confirmação explícita de suporte pela Z-API.
- A investigação do relato de mensagem enviada ao contato errado em 16 de setembro encontrou dois riscos concretos no código: a troca de conversa preservava rascunho/anexos enquanto o envio passava a usar o novo ID; e o webhook transformava `digits@lid` em dígitos de um suposto telefone. O painel agora reinicia o rascunho por conversa e exibe o destinatário junto ao compositor. O Worker preserva LID como endereço privado, vincula LID e telefone real sem fundir contatos conflitantes, bloqueia retentativa para número canônico não equivalente e compara o destinatário dos callbacks com o usado no novo envio; divergências geram alerta e auditoria sem registrar o número em logs. A consulta agregada somente de leitura ao D1 não permitiu atribuir o episódio a uma dessas causas: o histórico antigo não guarda o destinatário efetivo nem a conversa visível ao clicar. Dois contatos com formato não brasileiro tinham somente uma entrada e nenhuma saída, e não estiveram ativos no dia relatado. Nenhuma mensagem real foi enviada ou apagada durante a investigação.
- A migration `0014` passou no D1 local e remoto; após o deploy não restaram migrações pendentes. A validação local passou por 94 testes unitários, typecheck e build; os 38 cenários E2E chegaram ao fim sem falhas impressas em desktop/celular, mas o runner precisou ser interrompido depois porque o servidor de desenvolvimento permaneceu aberto. O responsável autorizou incluir na mesma publicação as mudanças locais anteriores de envio múltiplo de anexos. O commit `c2ea72b` foi enviado ao `main` institucional e publicado no Worker `59c53753-1c92-4949-a533-0f4a2771b272`. Em produção, a página e `/api/auth/status` responderam HTTP 200, as rotas novas de edição e exclusão responderam HTTP 401 sem sessão, e o Chromium mostrou a tela de login em 412 px sem overflow horizontal. Nenhuma mensagem real foi enviada, editada ou apagada durante a validação. Próximo passo operacional: observar uma mensagem real consentida com sessão autenticada e conferir o destinatário registrado e o callback; se houver novo relato de destinatário divergente, obter conversa e horário para correlacionar a auditoria e o ID da Z-API sem expor números em logs.
