...
- O modo autônomo está implementado localmente para conversas sem responsável, mas permanece desligado por padrão por `LUNA_AUTONOMOUS_ENABLED=false`. Antes de enviar, ele exige que a mensagem recebida ainda seja a mais recente, que ninguém tenha assumido a conversa e que a análise não peça revisão humana. A migration `0010_luna_autonomous_replies.sql` registra uma única tentativa por mensagem para impedir duplicidade. Não habilitar em produção sem decisão operacional explícita e validação controlada com destinatário autorizado.
...
- Em 14 de setembro de 2026, o atendimento autônomo da Luna foi concluído localmente com trava global desligada, deduplicação por mensagem, rechecagem de atribuição/ordem antes do envio, bloqueio por revisão humana, memória persistente e publicação em tempo real. A migration `0010` foi aplicada somente no D1 local. A validação passou por 50 testes unitários, incluindo o orquestrador autônomo, 22 E2E em desktop/celular, typecheck e build; um seletor E2E antigo foi tornado exato para eliminar ambiguidade entre as mensagens 40 e 408. A migration ainda não foi aplicada remotamente e o recurso não foi ativado nem publicado. Próximo passo: somente após autorização explícita, aplicar a migration, publicar mantendo a trava desligada e definir separadamente se haverá ativação controlada.
- O compositor do chat já enviava com Enter e preservava quebra de linha com Shift + Enter; em 14 de setembro de 2026, essa regra ganhou uma instrução visível e proteção para não enviar durante composição de caracteres pelo teclado. O comportamento passou a ter cobertura E2E em desktop e celular.
